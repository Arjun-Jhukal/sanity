...
export default function ResourceCard({ id, title, image, downloadNumber, slug, downloadLink }: Props) {
  return (
    <Card className="w-full max-w-fit border-0 !bg-transparent sm:max-w-[365px]">
      <Link href={downloadLink} target="_blank">
        <CardHeader className="flex-center flex-col gap-2.5 !p-9">
          <div className="h-fit w-full">
            <Image src={image} className="h-full rounder-md object-cover" width={384} height={440} alt={title} />
          </div>
      ...
      <CardContent className="flex-between mt-4 p-0">
        <div className="flex-center body-medium gap-1.5 text-white">
          <Image src={"/downloads.svg"} width={20} height={20} alt="Download Icon" />
          {downloadNumber}
        </div>
